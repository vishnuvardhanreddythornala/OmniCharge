...
class FallbackLogReplayerTest {
    ...
    void replayLogs_AllSuccess_FileDeleted() throws Exception {
        LogEvent e = new LogEvent(); e.setServiceName("payment-service"); e.setLevel("ERROR");
        e.setMessage("t"); e.setTimestamp(LocalDateTime.now());
        Path f = tempDir.resolve("fallback-buffer-payment-service.log");
        Files.writeString(f, MAPPER.writeValueAsString(e) + System.lineSeparator());

        invokeReplayLogs();
        verify(rabbitTemplate, times(1)).convertAndSend(anyString(), eq("log.payment-service"), any(LogEvent.class));
    ...
    void replayLogs_BrokerDown_FileRetained() throws Exception {
        ...
        invokeReplayLogs();
        assertTrue(Files.exists(tempDir.resolve("processing-buffer-payment-service.log")));
    }

    @Test
    void replayLogs_ExistingProcessingFile() throws Exception {
        LogEvent e = new LogEvent(); e.setServiceName("payment-service"); e.setLevel("INFO");
        e.setMessage("t"); e.setTimestamp(LocalDateTime.now());
        Path p = tempDir.resolve("processing-buffer-payment-service.log");
