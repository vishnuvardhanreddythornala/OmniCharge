    ...
    <div class="space-y-6 animate-fade-in relative max-w-full">
      ...
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-6 mb-8">
        <div>
          ...
          <p class="text-surface-400 text-sm">Monitor all platform recharge activity.</p>
        </div>
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6 animate-slide-up">
        ...
        <div class="glass-card p-5 flex flex-col justify-between border border-white/[0.05]">
          <div class="flex items-center gap-2 mb-2">
            ...
            <p class="text-[10px] text-surface-400 font-bold uppercase tracking-wider">Processing</p>
          </div>
          <span class="text-2xl font-display font-bold text-white">{{ processingCount() }}</span>
        </div>
      </div>
          ...
          <div class="flex items-center gap-1 w-full">
            @for (filter of statusFilters; track filter.key) {
              <button (click)="setCategory(filter.key)"
                      [class]="rechargeCategory() === filter.key ? 'bg-white/10 text-white shadow-sm' : 'text-surface-400 hover:text-white hover:bg-white/[0.02]'"
        ...
        @if (loading()) {
              ...
              <tbody class="divide-y divide-white/[0.05]">
                @for (rech of recharges(); track rech.rechargeId) {
                  <tr class="hover:bg-white/[0.02] transition-colors">
                    ...
                    <td class="px-6 py-4">
                      <div class="text-white font-medium text-sm">{{ rech.mobileNumber }}</div>
                      <div class="text-[10px] text-surface-400 mt-0.5">₹{{ rech.amount }} ({{ rech.planValidityDays }} Days)</div>
                    </td>
                    <td class="px-6 py-4">
            ...
            @for (rech of recharges(); track rech.rechargeId) {
              <div class="p-4 space-y-2.5">
                <div class="flex items-center justify-between">
                  <span class="font-mono text-xs text-white">{{ rech.mobileNumber }}</span>
                ...
                <div class="flex items-center justify-between text-xs">
                  <span class="text-surface-400">Recharge ID: <span class="font-mono text-surface-300">{{ rech.rechargeId }}</span></span>
                  <span class="font-semibold text-white">₹{{ rech.amount }}</span>
                </div>
          ...
          @if (recharges().length === 0) {
            <div class="p-16 text-center">
              ...
              <h3 class="text-lg font-semibold text-white mb-1.5">No recharges found</h3>
              ...
              @if (searchQuery || filterStartDate || filterEndDate) {
                <button (click)="clearFilters()" class="btn-ghost text-xs mt-4 border border-white/10">
                  Clear all filters
                </button>
              }
            </div>
