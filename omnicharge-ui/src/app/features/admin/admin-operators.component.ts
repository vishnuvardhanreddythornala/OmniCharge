  ...
  template: `
    <div class="animate-fade-in">
      ...
      <div class="flex items-center justify-between mb-6">
        <div>
          <h1 class="text-2xl font-display font-bold text-white">Operators</h1>
          <p class="text-sm text-surface-400 mt-1">Manage telecom operators &amp; their plans</p>
        </div>
        <button id="create-operator-btn" (click)="showCreateOperator.set(true)"
                class="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold bg-gradient-to-r from-omni-600 to-omni-500 text-white hover:from-omni-500 hover:to-omni-400 transition-all duration-300 shadow-lg shadow-omni-500/20 hover:shadow-omni-500/30 hover:-translate-y-0.5">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6 animate-slide-up">
        <div class="glass-card p-5 flex flex-col justify-between border border-white/[0.05]">
          <div class="flex items-center gap-2 mb-2">
            ...
            <p class="text-[10px] text-surface-400 font-bold uppercase tracking-wider">Total Operators</p>
          </div>
          <span class="text-3xl font-display font-bold text-white">{{ operators().length }}</span>
        </div>
        <div class="glass-card p-5 flex flex-col justify-between border border-white/[0.05]">
          <div class="flex items-center gap-2 mb-2">
            ...
            <p class="text-[10px] text-surface-400 font-bold uppercase tracking-wider">Active</p>
          </div>
          <span class="text-3xl font-display font-bold text-white">{{ activeOperatorCount() }}</span>
        </div>
        <div class="glass-card p-5 flex flex-col justify-between border border-white/[0.05]">
          <div class="flex items-center gap-2 mb-2">
            <div class="w-8 h-8 rounded-lg bg-surface-700/50 flex items-center justify-center border border-white/[0.08]">
              <svg class="w-4 h-4 text-surface-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636"/></svg>
            </div>
            <p class="text-[10px] text-surface-400 font-bold uppercase tracking-wider">Inactive</p>
          </div>
          <span class="text-3xl font-display font-bold text-white">{{ inactiveOperatorCount() }}</span>
        </div>
      </div>
          ...
          <div class="overflow-x-auto">
                  ...
                  <tr class="hover:bg-white/[0.02] transition-colors">
                    <td class="px-6 py-4">
                      <div class="flex items-center gap-3">
                        <div class="w-9 h-9 rounded-lg bg-gradient-to-br from-omni-600/30 to-accent-teal/30 border border-white/10 flex items-center justify-center text-white text-xs font-bold uppercase">
                          {{ op.code ? op.code.substring(0, 2) : 'OP' }}
                        </div>
                        <div class="font-medium text-white">{{ op.name || 'Unnamed Operator' }}</div>
                      </div>
                    </td>
                    <td class="px-6 py-4">
                      <span class="px-2 py-1 rounded bg-white/[0.05] text-xs font-mono border border-white/10 text-surface-300">{{ op.code || 'N/A' }}</span>
                    </td>
                    <td class="px-6 py-4">
                      @if (op.category) {
                    ...
                    <td class="px-6 py-4">
                      <span class="text-sm font-semibold text-white">{{ op.planCount || 0 }}</span>
                    ...
                    <td class="px-6 py-4">
                      <div class="flex flex-col">
                        @if (op.lastModifiedDate) {
                          <span class="text-xs text-surface-200 font-medium">{{ formatDate(op.lastModifiedDate) }}</span>
                        } @else {
            ...
            @if (operators().length === 0) {
              <div class="p-12 text-center">
                ...
                <p class="text-surface-400 font-medium">No operators found.</p>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4" (click)="showCreateOperator.set(false)">
          ...
          <div class="relative w-full max-w-lg glass-card p-0 border border-white/[0.08] shadow-2xl animate-scale-in" (click)="$event.stopPropagation()">
            ...
            <div class="flex items-center justify-between px-6 py-5 border-b border-white/[0.05]">
              <h2 class="text-lg font-display font-bold text-white">Create New Operator</h2>
              <button (click)="showCreateOperator.set(false)" class="text-surface-400 hover:text-white transition-colors">
            ...
            <div class="p-6 space-y-5">
              <div>
                <label class="block text-xs font-semibold text-surface-400 uppercase tracking-wider mb-2">Operator Name *</label>
                <input type="text" [(ngModel)]="newOperator.name" placeholder="e.g. Jio"
                       class="w-full px-4 py-3 rounded-xl bg-white/[0.04] border border-white/10 text-white placeholder-surface-500 text-sm focus:outline-none focus:border-omni-500/50 focus:ring-1 focus:ring-omni-500/30 transition-all"/>
              </div>
              <div class="grid grid-cols-2 gap-4">
                <div>
                  <label class="block text-xs font-semibold text-surface-400 uppercase tracking-wider mb-2">Code *</label>
                  <input type="text" [(ngModel)]="newOperator.code" placeholder="e.g. JIO"
                         class="w-full px-4 py-3 rounded-xl bg-white/[0.04] border border-white/10 text-white placeholder-surface-500 text-sm focus:outline-none focus:border-omni-500/50 focus:ring-1 focus:ring-omni-500/30 transition-all"/>
                </div>
                <div>
                  <label class="block text-xs font-semibold text-surface-400 uppercase tracking-wider mb-2">Category *</label>
                  <select [(ngModel)]="newOperator.category"
                          class="w-full px-4 py-3 rounded-xl bg-white/[0.04] border border-white/10 text-white text-sm focus:outline-none focus:border-omni-500/50 focus:ring-1 focus:ring-omni-500/30 transition-all appearance-none cursor-pointer">
              ...
              <div>
                <label class="block text-xs font-semibold text-surface-400 uppercase tracking-wider mb-2">Logo URL (optional)</label>
            ...
            <div class="flex items-center justify-end gap-3 px-6 py-4 border-t border-white/[0.05]">
              <button (click)="showCreateOperator.set(false)" class="px-4 py-2.5 rounded-xl text-sm font-medium text-surface-400 hover:text-white hover:bg-white/[0.04] transition-all">Cancel</button>
              <button (click)="createOperator()" [disabled]="creatingOperator()"
                      class="px-5 py-2.5 rounded-xl text-sm font-semibold bg-gradient-to-r from-omni-600 to-omni-500 text-white hover:from-omni-500 hover:to-omni-400 transition-all disabled:opacity-50 disabled:cursor-not-allowed">
      ...
      @if (showEditOperator()) {
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4" (click)="showEditOperator.set(false)">
          ...
          <div class="relative w-full max-w-lg glass-card p-0 border border-white/[0.08] shadow-2xl animate-scale-in" (click)="$event.stopPropagation()">
            <!-- Modal Header -->
            <div class="flex items-center justify-between px-6 py-5 border-b border-white/[0.05]">
              <h2 class="text-lg font-display font-bold text-white">Edit Operator</h2>
              <button (click)="showEditOperator.set(false)" class="text-surface-400 hover:text-white transition-colors">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
              </button>
            </div>
            <!-- Modal Body -->
            ...
            <!-- Modal Footer -->
            <div class="flex items-center justify-end gap-3 px-6 py-4 border-t border-white/[0.05]">
              <button (click)="showEditOperator.set(false)" class="px-4 py-2.5 rounded-xl text-sm font-medium text-surface-400 hover:text-white hover:bg-white/[0.04] transition-all">Cancel</button>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4" (click)="showDeleteConfirm.set(false)">
          ...
          <div class="relative w-full max-w-md glass-card p-0 border border-accent-rose/20 shadow-2xl animate-scale-in" (click)="$event.stopPropagation()">
            ...
            <div class="flex items-center gap-3 px-6 py-5 border-b border-white/[0.05]">
              ...
              <div>
                <h2 class="text-lg font-display font-bold text-white">Delete Operator</h2>
            ...
            <div class="p-6">
              <p class="text-sm text-surface-300">
                Are you sure you want to delete <span class="font-semibold text-white">{{ operatorToDelete()?.name }}</span>? 
                This will also remove all associated plans.
              </p>
            </div>
            <!-- Modal Footer -->
            <div class="flex items-center justify-end gap-3 px-6 py-4 border-t border-white/[0.05]">
              <button (click)="showDeleteConfirm.set(false)" class="px-4 py-2.5 rounded-xl text-sm font-medium text-surface-400 hover:text-white hover:bg-white/[0.04] transition-all">Cancel</button>
              <button (click)="deleteOperator()" [disabled]="deletingOperator()"
                      class="px-5 py-2.5 rounded-xl text-sm font-semibold bg-accent-rose text-white hover:bg-accent-rose/90 transition-all disabled:opacity-50 disabled:cursor-not-allowed">
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4" (click)="showAddPlan.set(false)">
          ...
          <div class="relative w-full max-w-2xl glass-card p-0 border border-white/[0.08] shadow-2xl animate-scale-in max-h-[90vh] overflow-y-auto" (click)="$event.stopPropagation()">
            ...
            <div class="flex items-center justify-between px-6 py-5 border-b border-white/[0.05] sticky top-0 bg-omni-950/95 backdrop-blur-xl z-10">
              <div>
                <h2 class="text-lg font-display font-bold text-white">Add Plan</h2>
                <p class="text-xs text-surface-400 mt-0.5">for <span class="text-omni-400 font-semibold">{{ selectedOperator()?.name }}</span></p>
              </div>
              <button (click)="showAddPlan.set(false)" class="text-surface-400 hover:text-white transition-colors">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
              </button>
            </div>
            <!-- Modal Body -->
            <div class="p-6 space-y-5">
              <div class="grid grid-cols-2 gap-4">
                <div class="col-span-2">
                  <label class="block text-xs font-semibold text-surface-400 uppercase tracking-wider mb-2">Plan Name *</label>
                  <input type="text" [(ngModel)]="newPlan.planName" placeholder="e.g. Unlimited Data Pack"
                         class="w-full px-4 py-3 rounded-xl bg-white/[0.04] border border-white/10 text-white placeholder-surface-500 text-sm focus:outline-none focus:border-omni-500/50 focus:ring-1 focus:ring-omni-500/30 transition-all"/>
                </div>
                <div>
                  <label class="block text-xs font-semibold text-surface-400 uppercase tracking-wider mb-2">Price (₹) *</label>
                  <input type="number" [(ngModel)]="newPlan.price" placeholder="299"
                         class="w-full px-4 py-3 rounded-xl bg-white/[0.04] border border-white/10 text-white placeholder-surface-500 text-sm focus:outline-none focus:border-omni-500/50 focus:ring-1 focus:ring-omni-500/30 transition-all"/>
                </div>
                <div>
                  <label class="block text-xs font-semibold text-surface-400 uppercase tracking-wider mb-2">Validity (Days) *</label>
                  <input type="number" [(ngModel)]="newPlan.validityDays" placeholder="28"
                         class="w-full px-4 py-3 rounded-xl bg-white/[0.04] border border-white/10 text-white placeholder-surface-500 text-sm focus:outline-none focus:border-omni-500/50 focus:ring-1 focus:ring-omni-500/30 transition-all"/>
                </div>
                <div>
                  <label class="block text-xs font-semibold text-surface-400 uppercase tracking-wider mb-2">Data Limit</label>
                  <input type="text" [(ngModel)]="newPlan.dataLimit" placeholder="e.g. 2GB/day"
                         class="w-full px-4 py-3 rounded-xl bg-white/[0.04] border border-white/10 text-white placeholder-surface-500 text-sm focus:outline-none focus:border-omni-500/50 focus:ring-1 focus:ring-omni-500/30 transition-all"/>
                </div>
                <div>
                  <label class="block text-xs font-semibold text-surface-400 uppercase tracking-wider mb-2">Call Benefit</label>
                  <input type="text" [(ngModel)]="newPlan.callBenefit" placeholder="e.g. Unlimited"
                         class="w-full px-4 py-3 rounded-xl bg-white/[0.04] border border-white/10 text-white placeholder-surface-500 text-sm focus:outline-none focus:border-omni-500/50 focus:ring-1 focus:ring-omni-500/30 transition-all"/>
                </div>
                <div>
                  <label class="block text-xs font-semibold text-surface-400 uppercase tracking-wider mb-2">SMS Benefit</label>
                ...
                <div class="col-span-2">
                  <label class="block text-xs font-semibold text-surface-400 uppercase tracking-wider mb-2">Additional Benefits</label>
                  <textarea [(ngModel)]="newPlan.additionalBenefits" placeholder="e.g. Free access to JioTV, JioCinema..."
                            rows="3"
                            class="w-full px-4 py-3 rounded-xl bg-white/[0.04] border border-white/10 text-white placeholder-surface-500 text-sm focus:outline-none focus:border-omni-500/50 focus:ring-1 focus:ring-omni-500/30 transition-all resize-none"></textarea>
                </div>
              </div>
            </div>
            <!-- Modal Footer -->
            <div class="flex items-center justify-end gap-3 px-6 py-4 border-t border-white/[0.05] sticky bottom-0 bg-omni-950/95 backdrop-blur-xl">
              <button (click)="showAddPlan.set(false)" class="px-4 py-2.5 rounded-xl text-sm font-medium text-surface-400 hover:text-white hover:bg-white/[0.04] transition-all">Cancel</button>
              <button (click)="createPlan()" [disabled]="creatingPlan()"
                      class="px-5 py-2.5 rounded-xl text-sm font-semibold bg-gradient-to-r from-accent-teal to-accent-emerald text-white hover:shadow-lg hover:shadow-accent-teal/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed">
