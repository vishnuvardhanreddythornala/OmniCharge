...
export class RechargeFlowComponent implements OnInit, HasUnsavedChanges {
  ...
  /** Filtered plans based on active tab */
  filteredPlans = computed(() => {
    const plans = this.operatorService.plans();
    if (this.activeCategory() === 'ALL') return plans;
    return plans.filter(p => p.category === this.activeCategory());
  });
