      ...
      <div class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
        <div class="glass-card w-full max-w-sm p-6 sm:p-8 animate-scale-in relative border-omni-500/30 shadow-glow">
          ...
          <div class="text-center mb-6">
            ...
            <h2 class="text-xl font-display font-bold text-surface-900 mb-2">{{ config.title }}</h2>
          ...
          <div class="flex gap-3">
            <button (click)="dialogService.close(false)" class="btn-secondary flex-1 !py-3 text-sm border border-surface-200 hover:border-surface-300">
              {{ config.cancelLabel }}
            </button>
