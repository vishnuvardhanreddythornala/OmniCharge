...
class FallbackLogReplayerTest {
    ...
    void setUp() {
        ...
        ReflectionTestUtils.setField(replayer, "serviceName", "recharge-service");
    ...
    void replayLogs_AllSuccess_FileDeleted() throws Exception {
        LogEvent e = new LogEvent(); e.setServiceName("recharge-service"); e.setLevel("ERROR");
        ...
        invokeReplayLogs();
        verify(rabbitTemplate, times(1)).convertAndSend(anyString(), eq("log.recharge-service"), any(LogEvent.class));
        assertFalse(Files.exists(tempDir.resolve("processing-buffer-recharge-service.log")));
    }
    ...
    void replayLogs_BrokerDown_FileRetained() throws Exception {
        ...
        invokeReplayLogs();
        assertTrue(Files.exists(tempDir.resolve("processing-buffer-recharge-service.log")));
    }

    @Test
    void replayLogs_ExistingProcessingFile() throws Exception {
        LogEvent e = new LogEvent(); e.setServiceName("recharge-service"); e.setLevel("INFO");
        e.setMessage("t"); e.setTimestamp(LocalDateTime.now());
        Path p = tempDir.resolve("processing-buffer-recharge-service.log");
